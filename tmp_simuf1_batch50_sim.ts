...
const generateParticipants = (raceId: string, participantsCount: number): SimuF1Entry[] => {
...
const byCircuit = new Map<string, {
  races: number;
  cars: number;
  dnfs: number;
  audaceAttempts: number;
  audaceSuccess: number;
  defenseSuccess: number;
  pneusFailures: number;
}>();
...
for (const race of races) {
  ...
  lines.push(
    `AUDACE ${audaceSuccess}/${Math.max(1, audaceAttempts)} (${((audaceSuccess / Math.max(1, audaceAttempts)) * 100).toFixed(1)}%) | DEFENSE succes=${defenseSuccess} | PNEUS echecs=${pneusFailures}`
  );
...
for (const [circuitName, s] of byCircuit.entries()) {
  lines.push(
    `- ${circuitName}: courses=${s.races}, voitures=${s.cars}, DNF=${s.dnfs} (${((s.dnfs / Math.max(1, s.cars)) * 100).toFixed(1)}%), AUDACE=${s.audaceSuccess}/${Math.max(1, s.audaceAttempts)} (${((s.audaceSuccess / Math.max(1, s.audaceAttempts)) * 100).toFixed(1)}%), DEFENSE succes=${s.defenseSuccess}, PNEUS echecs=${s.pneusFailures}`
  );
}
lines.push("");
...
const reportPath = resolve(process.cwd(), "simuf1_rapport_simulation_50_courses_2026-04-30.txt");
