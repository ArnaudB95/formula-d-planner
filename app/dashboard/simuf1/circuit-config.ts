import type { SimuF1CarSetup } from "./types";

export type SimuF1StatKey = keyof Pick<
  SimuF1CarSetup,
  "bloc" | "grip" | "audace" | "defense" | "endurance" | "pneus"
>;

export type SimuF1CircuitProfile = "rapide" | "technique" | "mixte" | "imprevisible" | "urbain";

export type SimuF1CircuitConfig = {
  week: number;
  circuitName: string;
  profile: SimuF1CircuitProfile;
  boosted: SimuF1StatKey[];
  penalized: SimuF1StatKey[];
};

const byWeek: Record<number, SimuF1CircuitConfig> = {
...
export const weekNumberFromWeekKey = (weekKey: string) => {
...
export const getCircuitConfigForWeekKey = (weekKey: string) => {
  const week = weekNumberFromWeekKey(weekKey);
  return byWeek[week] || byWeek[1];
};

export const getStatModifier = (config: SimuF1CircuitConfig, stat: SimuF1StatKey): "boosted" | "penalized" | "neutral" => {
  if (config.boosted.includes(stat)) return "boosted";
  if (config.penalized.includes(stat)) return "penalized";
  return "neutral";
};

export const getStatMultiplier = (config: SimuF1CircuitConfig, stat: SimuF1StatKey) => {
  const modifier = getStatModifier(config, stat);
  if (modifier === "boosted") return 1.5;
  if (modifier === "penalized") return 0.5;
  return 1;
};

export const profileLabel = (profile: SimuF1CircuitProfile) => {
  if (profile === "rapide") return "Rapide";
  if (profile === "technique") return "Technique";
  if (profile === "mixte") return "Mixte";
  if (profile === "imprevisible") return "Imprevisible";
  return "Urbain";
};
