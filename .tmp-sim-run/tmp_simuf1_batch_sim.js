...
const generateParticipants = (raceId, seasonYear, count) => {
...
const races = weekKeys.map((weekKey, idx) => {
...
const summarizeRace = (pack) => {
    const totalCars = pack.result.cars.length;
    const dnfs = pack.result.cars.filter((c) => c.dnf).length;
    const finishers = totalCars - dnfs;
    const avgPointsTop3 = pack.result.cars.slice(0, 3).reduce((acc, c) => acc + c.points, 0) / Math.max(1, Math.min(3, totalCars));
    ...
    const participantStats = pack.participants.flatMap((p) => p.cars).reduce((acc, car) => {
        acc.bloc += car.bloc;
        acc.grip += car.grip;
...
for (const race of raceSummaries) {
    ...
    lines.push(`  BLOC ${race.summary.avgStats.bloc.toFixed(2)} | GRIP ${race.summary.avgStats.grip.toFixed(2)} | AUDACE ${race.summary.avgStats.audace.toFixed(2)} | DEFENSE ${race.summary.avgStats.defense.toFixed(2)} | ENDURANCE ${race.summary.avgStats.endurance.toFixed(2)} | PNEUS ${race.summary.avgStats.pneus.toFixed(2)}`);
    lines.push("Classement final:");
    race.result.cars.forEach((car) => {
        const status = car.dnf ? `DNF (tour ${car.dnfLap})` : "Arrivee";
        lines.push(`  P${car.position} - ${car.pilotName} [${car.teamName}] | ${status} | points=${car.points}`);
    });
    lines.push("Indicateurs course:");
    lines.push(`  DNF: ${race.summary.dnfs}/${race.summary.totalCars} (${race.summary.dnfRate.toFixed(1)}%) | tentatives audace=${race.summary.audaceAttempts} | audace reussie=${race.summary.audaceSuccess} | defenses reussies=${race.summary.defenseSuccess} | pertes adherence pneus=${race.summary.pneusFailures}`);
    lines.push("");
}
lines.push("=== ANALYSE SYNTHETIQUE ===");
lines.push(`Au global, ${globalCars} voitures ont pris le depart et ${globalDnfs} ont abandonne (${((globalDnfs / Math.max(1, globalCars)) * 100).toFixed(1)}%).`);
lines.push(`Le ratio de succes des attaques AUDACE est de ${globalAudaceSuccess}/${Math.max(1, globalAudaceAttempts)} (${((globalAudaceSuccess / Math.max(1, globalAudaceAttempts)) * 100).toFixed(1)}%).`);
lines.push("Constats:");
...
const reportPath = (0, node_path_1.resolve)(process.cwd(), "simuf1_rapport_simulation_2026-04-30.txt");
